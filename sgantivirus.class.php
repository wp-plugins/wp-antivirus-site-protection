...
class SGAntiVirus_module
{
	...
	public static function scan($check_session = true, $show_results = true)
	{
		...
		include_once(dirname(__FILE__).DIRSEP.'HttpClient.class.php');
		...
		if ($check_session)
		{
			...
			if ($_SESSION['scan']['session_id'] != $session_id)
			{
		...
		$scan_path = base64_decode(trim($_POST['scan_path']));
		...
		$session_report_key = trim($_POST['session_report_key']); 
			...
			$error_msg = 'TMP folder - '.dirname(__FILE__).DIRSEP.'tmp'.DIRSEP;
			if (self::$debug) self::DebugLog($error_msg);
		...
		if ($ssh_flag)
		{
			...
			$cmd = 'cd '.$scan_path.''."\n".'tar -czf '.dirname(__FILE__).DIRSEP.'tmp'.DIRSEP.'pack.tar -T '.$collected_filelist;
			$output = array();
			$result = exec($cmd, $output);
			
			if (file_exists(dirname(__FILE__).DIRSEP.'tmp'.DIRSEP.'pack.tar') === false) 
			{
				$ssh_flag = false;
		...
		if ($ssh_flag)
		{
	 		$archive_filename = dirname(__FILE__).DIRSEP."tmp".DIRSEP."pack.tar";
	 		$archive_format = 'tar';
		} else {
			$archive_filename = dirname(__FILE__).DIRSEP."tmp".DIRSEP."pack.zip";
			$archive_format = 'zip';
		}
